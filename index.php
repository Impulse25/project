...
<?php if (!$isLoggedIn): ?>
<script>
// Для неавторизованных: перехватываем клики в сайдбаре → показываем логин-модал
  ...
  document.querySelectorAll('.sidebar .nav-item').forEach(function(link) {
    ...
    if (href && href !== '/' && href !== '#') {
      link.addEventListener('click', function(e) {
        e.preventDefault();
        showLogin(href, link.querySelector('span')?.textContent?.trim() || '');
      });
    }
  });
});
...
<script>
// sidebar + dark mode: обрабатывает portal_sidebar.php
