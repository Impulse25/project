...
// ВАЖНО: фильтр по ПЦК раньше брался из ta.pcc_head_id (кто из председателей
// назначил преподавателя). Так как назначения преподавателей больше не
// участвуют в выгрузке предметов с контрольными работами, этот фильтр
// сейчас отключён — фильтровать по ПЦК тут неоткуда без отдельной связи
// "специальность/группа -> председатель ПЦК". Если такая связь есть в БД
// (например, edu_specialties.pcc_head_id или похожая), скажите — добавлю JOIN.
$filterPccId = null;
$whereClauseGroups = '';

// Запрос
// ВАЖНО: источник — учебный план (edu_curriculum_modules), а НЕ назначения
// преподавателей. Предмет с контрольной работой попадает в выгрузку
// независимо от того, назначен ли на него преподаватель.
// Семестр берём из edu_curriculum_distribution (разбивка модуля по
// семестрам в рамках учебного плана), а не из umr_teacher_assignments.
//
// Группа считается "учащейся" в учебном году $yearStart, если она уже
// поступила (year_started <= yearStart) и её обучение ещё не завершилось
// (year_started + длительность обучения по учебному плану > yearStart).
$stmt = $pdo->prepare("
    SELECT
