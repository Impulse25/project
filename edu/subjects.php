...
if (isset($_GET['delete'])) {
    try {
        $pdo->prepare("DELETE FROM edu_subjects WHERE id = ?")->execute([(int)$_GET['delete']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    $hours_total = trim($_POST['hours_total'] ?? '') !== '' ? (int)$_POST['hours_total'] : null;

    if ($code === '' || $name_ru === '') {
        $message     = 'Код и наименование (рус.) обязательны.';
        $messageType = 'error';
    } elseif ($hours_total !== null && ($hours_total < 0 || $hours_total > 2000)) {
        $message     = 'Часов: от 0 до 2000.';
        $messageType = 'error';
    } else {
        try {
            if ($_POST['action'] === 'create') {
                $pdo->prepare("INSERT INTO edu_subjects (code, name_ru, name_kz, hours_total) VALUES (?,?,?,?)")
                    ->execute([$code, $name_ru, $name_kz ?: null, $hours_total]);
                $message = "Дисциплина «{$name_ru}» добавлена.";
            } else {
                $editId = (int)$_POST['edit_id'];
                $pdo->prepare("UPDATE edu_subjects SET code=?, name_ru=?, name_kz=?, hours_total=? WHERE id=?")
                    ->execute([$code, $name_ru, $name_kz ?: null, $hours_total, $editId]);
                $message = "Дисциплина «{$name_ru}» обновлена.";
            }
            $messageType = 'success';
        } catch (PDOException $e) {
...
$editRow = null;
if (isset($_GET['edit'])) {
    $stmt = $pdo->prepare("SELECT * FROM edu_subjects WHERE id = ?");
    $stmt->execute([(int)$_GET['edit']]);
    $editRow = $stmt->fetch(PDO::FETCH_ASSOC);
}

$rows  = $pdo->query("SELECT * FROM edu_subjects ORDER BY code")->fetchAll(PDO::FETCH_ASSOC);
$total = count($rows);

$pageTitle       = 'Дисциплины — СВГТК Портал';
$activeNav       = 'edu';
  ...
  <main class="page-content">
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-subtitle">Справочник учебных дисциплин</p>
      </div>
    ...
    <div class="card">
      <div class="card-header">
        ...
        <?php if ($editRow): ?>
        <a href="subjects.php" class="btn btn-outline" style="padding:.3rem .75rem;font-size:.8125rem">Отмена</a>
        <?php endif ?>
      </div>
      <div class="card-body">
        <form method="POST" action="subjects.php">
          ...
          <div class="form-grid">
            ...
            <div class="form-group">
              <label for="f_hours">Всего часов (≤ 2000)</label>
              <input type="number" id="f_hours" name="hours_total" min="0" max="2000"
                     placeholder="Например: 120"
                     value="<?= htmlspecialchars($editRow['hours_total'] ?? '') ?>">
            </div>
          </div>
          <div style="margin-top:1.25rem">
            <button type="submit" class="btn btn-primary">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>
              <?= $editRow ? 'Сохранить изменения' : 'Добавить дисциплину' ?>
            </button>
          </div>
        </form>
      </div>
    ...
    <div class="card">
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th>Часов</th><th style="text-align:right">Действия</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr>
              ...
              <td>
                <?php if ($r['hours_total'] !== null): ?>
                <span class="badge badge-amber"><?= $r['hours_total'] ?> ч.</span>
                <?php else: ?>
              ...
              <td>
                <div class="action-btns" style="justify-content:flex-end">
                  <a href="subjects.php?edit=<?= $r['id'] ?>" class="btn btn-outline" style="padding:.3rem .6rem;font-size:.8125rem">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                  </a>
                  <a href="subjects.php?delete=<?= $r['id'] ?>"
                     class="btn btn-danger" style="padding:.3rem .6rem;font-size:.8125rem"
                     onclick="return confirm('Удалить дисциплину «<?= addslashes(htmlspecialchars($r['name_ru'])) ?>»?')">
      ...
      <div class="empty-state">
        ...
        <div class="empty-state-title">Нет дисциплин</div>
        <div class="empty-state-sub">Добавьте первую дисциплину с помощью формы выше</div>
      </div>
      <?php endif ?>
    </div>
