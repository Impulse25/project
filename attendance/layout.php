<?php
/**
 * layout.php — HTML-обёртка: DOCTYPE, <head>, sidebar, topbar, footer
 * Контент вкладок подключается через tabs.php
 */
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Учёт посещаемости — СВГТК Портал [v4]</title>
  ...
  <!-- Тема применяется сразу, до рендера — иначе будет мигание белого экрана -->
  <script>
    (function(){
  ...
  <header class="topbar">
    <div class="topbar-left">
      ...
      <div class="breadcrumb">
        ...
        <span class="breadcrumb-current">Учёт посещаемости</span>
      </div>
    </div>
...
<a href="help.php" target="_blank"
  ...
  <footer class="page-footer">
    <span>СВГТК Портал · Модуль «Учёт посещаемости»</span>
    <span>Разработано в рамках дипломной работы · СВГТК им. Абая Кунанбаева · <?= date('Y') ?></span>
  </footer>
...
function exportRapCSV() {
  ...
  table.querySelectorAll('tr').forEach(tr => {
    ...
    tr.querySelectorAll('th, td').forEach(td => {
      ...
      cells.push('"' + clone.textContent.trim().replace(/\s+/g,' ').replace(/"/g,'""') + '"');
    });
