...
<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:60vh;gap:var(--space-4);text-align:center">
    ...
    <div class="mode-switch-wrap">
      ...
      <label class="mode-switch" title="Переключить режим преподавателя / зав. отделения">
        <input type="checkbox" <?= ($attendanceMode ?? 'teacher') === 'department' ? 'checked' : '' ?>
               onchange="document.getElementById('attendanceModeInputEmpty').value=this.checked?'department':'teacher'; this.form.submit();">
  ...
  <div>
    ...
    <div style="color:var(--color-text-muted);max-width:420px;line-height:1.6">
      Для вашей учётной записи не найдены доступные группы.<br>
      Для преподавателя нужно заполнить <code>curator_id</code> у группы.<br>
      Для зав. отделения нужно указать <code>users.is_department_head = 1</code> и <code>users.head_department_id</code>.
    </div>
  </div>
</div>
<?php else: ?>
...
<!-- Заголовок модуля -->
<div class="page-header attendance-module-header">
  <div>
    <h1 class="page-title">Учёт посещаемости</h1>
    <p class="page-subtitle attendance-current-info">
  ...
  <form method="get" class="attendance-filters-form" style="display:flex;flex-wrap:wrap;gap:var(--space-4);align-items:flex-end;width:100%">
    ...
    <div class="form-group">
      <label class="form-label">
        Группа
        <span style="font-size:10px;color:var(--color-text-faint);font-weight:400">(<?= htmlspecialchars($groupsLabel ?? 'ваши группы') ?>)</span>
      </label>
    ...
    <div class="form-group">
      ...
      <select name="student" class="form-control" onchange="this.form.submit()" style="min-width:220px">
        <option value="0">Все студенты группы</option>
        <?php foreach ($allGroupStudents as $stOpt): ?>
        <option value="<?= (int)$stOpt['id'] ?>" <?= $selectedStudent === (int)$stOpt['id'] ? 'selected' : '' ?>>
          <?= htmlspecialchars(trim($stOpt['surname'].' '.$stOpt['first_name'].' '.$stOpt['patronymic'])) ?>
        </option>
        <?php endforeach ?>
      </select>
    </div>
...
<div class="kpi-grid">
  <div class="kpi-card">
    ...
    <div class="kpi-label"><?= $selectedStudent > 0 ? 'Выбран студент' : 'Всего студентов' ?></div>
      ...
      <form method="get" class="rap-filters">
          ...
          <div class="form-group">
            ...
            <select name="period" class="form-control" style="min-width:130px" onchange="this.form.submit()">
              ...
              <option value="custom"   <?= $reportPeriod==='custom'   ?'selected':'' ?>>Свой период</option>
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <input type="date" name="date_from" class="form-control"
                   value="<?= htmlspecialchars($customDateFrom) ?>" onchange="this.form.period.value='custom';this.form.submit()">
          </div>

          <div class="form-group">
            <label class="form-label">Дата по</label>
            <input type="date" name="date_to" class="form-control"
                   value="<?= htmlspecialchars($customDateTo) ?>" onchange="this.form.period.value='custom';this.form.submit()">
          </div>
      ...
      <div class="card" style="margin-bottom:var(--space-6)">
        <div class="card-header" style="flex-wrap:wrap;gap:var(--space-3)">
          <span class="card-title">
            ...
            &nbsp;·&nbsp;
            <?php
              $periodLabels = ['week'=>'Неделя','month'=>'Месяц','semester'=>'Семестр','year'=>'Учебный год','custom'=>'Свой период'];
          ...
          <div style="padding:var(--space-5);display:flex;flex-direction:column;gap:var(--space-4)">
            ...
            <div class="form-group">
              ...
              <select class="form-control" id="docStudent">
                ...
                <?php foreach ($allGroupStudents as $st): ?>
                <option value="<?= (int)$st['id'] ?>"><?= htmlspecialchars($st['surname'].' '.$st['first_name'].' '.$st['patronymic']) ?></option>
                <?php endforeach ?>
              </select>
            </div>
